Migrations.add({
    ...
    up: function () {
        Meteor.candidates.find({}).forEach(function(can) {
            var candidateInfo = {
            ...
            Meteor.applications.update({candidateId: can.candidateId}, {$set: {candidateInfo: candidateInfo}}, {multi:true});
        });
    },
    down: function () {
        console.log("down to version 7");
        Meteor.applications.update({}, {$unset: {candidateInfo: ''}}, {multi: true});
    }
});