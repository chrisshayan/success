...
CRON_VNW.syncJob = function (companyId, userId) {
    try {
        //get all jobs
        ...
        jobFromSQL.forEach(function (job) {
            ...
            if (!current) {
                processJob(jobId, companyId);

            } else if (current.updatedAt.toString() != formatDatetimeFromVNW(job.updatedAt).toString()
                || current.createdAt.toString() != formatDatetimeFromVNW(job.createdAt).toString()) {

                console.log('update Job :', job.jobId);
                console.log('previous info create : %s, update :%s', current.createdAt.toString(), current.updatedAt.toString());
                console.log('new info create : %s, update :%s', job.createdAt.toString(), job.updatedAt.toString());

                processJob(jobId, companyId, true);
            }

        });
        var ids = _.pluck(jobFromSQL, 'jobId');
        var chunks = _.chunk(ids, 5);
        chunks.forEach(function (chunk) {
            syncApplication(chunk, companyId);
        });
...
function syncApplication(jobIds, companyId) {
    var aQuery = sprintf(VNW_QUERIES.cronApplicationsUpdate, jobIds, jobIds);
    ...
    if (groupedApp['1']) {
        ...
        syncApplicationOnline(onlineIds, companyId);

    }

    if (groupedApp['2']) {
        var directIds = _.pluck(groupedApp['2'], 'typeId').join(',');
        syncApplicationDirect(directIds, companyId)
    }
...
function syncApplicationOnline(ids, companyId) {
...
function syncApplicationDirect(ids, companyId) {
...
CRON_VNW.startupSync = function () {
    ...
    var users = Meteors.users.find({}, {fields: {vnwId: 1, companyId: 1}}).fetch();
    users && users.forEach(function (user) {
        console.log('user: ', user);
        CRON_VNW.syncJob(user.companyId, user.vnwId);
    });

    console.log(' sync done!');
};

function processJob(jobId, companyId, isUpdate) {


    var getJobQuery = sprintf(VNW_QUERIES.pullJob, jobId);
    ...
    cJobs.forEach(function (row) {
        ...
        var job = {
            ...
            benefits: '',
            recruiterEmails: (row.emailaddress) ? _.unique(row.emailaddress.toLowerCase().match(/[A-Za-z\.0-9_]+@[a-zA-Z\.0-9_]+/g)) : '',
            skills: [],
            vnwData: row,
