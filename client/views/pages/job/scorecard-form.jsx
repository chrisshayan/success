ScoreCardFormModel = Astro.Class({
	name: 'ScoreCardFormModel',
	fields: {
		overall: {
			type: 'number',
			validator: [
				Validators.required(null, 'Please choose Overall recommendation')
			]
		}
	}
});
...
ScoreCardForm = React.createClass({
	...
	handleSave(e) {
		...
		const model = new ScoreCardFormModel(data);
		if (model.validate()) {
			...
			// scroll to errors
			const scrollTop = $(this.refs.overall).offset().top - 35;
			$("html, body")
				.stop()
				.animate({scrollTop: scrollTop || 0}, '500', 'swing');
		}
	},
	...
	render() {
		return (
			<div className="scorecard-form">
				...
				<div className="scorecard-overall" ref="overall">
					<div className="row">
						<div className="col-md-12">
							<h3>
								Overall recommendation
								<span>&nbsp;</span>
								<small><b>(required)</b> Did the candidate pass the interview?</small>
							</h3>
						</div>
					</div>

					<ScoreCardOverall onChange={this.handleChangeOverall} overall={this.state.overall}/>
					{this.state.errors.overall && (
